...
export default function SocioReservas() {
  ...
  const verDetalle = async (idReserva) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/socio/reservas/${idReserva}`, {
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!response.ok) {
        setError(data.message || "No se pudo cargar el detalle de la reserva.");
        return;
      }
  ...
  const cancelarReserva = async () => {
    ...
    const confirmar = window.confirm(
      "¿Seguro que deseas cancelar esta reserva? Esta acción cambiará su estatus a Cancelada."
    );
            ...
            <tbody>
              {reservas.map((reserva) => (
                <tr key={reserva.id_reserva} className="border-t">
                  ...
                  <td className="p-4">{getEspacioNombre(reserva)}</td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl p-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <InfoItem label="Espacio" value={getEspacioNombre(reservaDetalle)} />
              <InfoItem label="Fecha" value={formatDate(reservaDetalle.fecha)} />
              <InfoItem
                label="Hora inicio"
                value={reservaDetalle.hora_inicio}
              />
              <InfoItem label="Hora fin" value={reservaDetalle.hora_fin} />
              <InfoItem label="Estatus" value={reservaDetalle.estatus} />
              <InfoItem
                label="No-show"
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              {reservaDetalle.estatus === "Activa" && (
                <button
                  ...
                  disabled={cancelando}
                  className="bg-red-600 hover:bg-red-700 disabled:bg-red-300 text-white rounded-lg px-4 py-2"
                >

...
const Actividades = () => {
  ...
  const getAgenda = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(result.message || "Error al cargar agenda");
      }
    ...
    return agenda.filter(
      (s) =>
        ...
        (s.disciplina?.nombre_disciplina &&
          s.disciplina.nombre_disciplina.toLowerCase().includes(q)) ||
        (s.instructor?.nombre_completo &&
          s.instructor.nombre_completo.toLowerCase().includes(q))
    );
  }, [agenda, searchQuery]);
  ...
  const agendaTotalPages = Math.max(
    1,
    Math.ceil(filteredAgenda.length / ITEMS_PER_PAGE)
  );
  ...
  const reservasTotalPages = Math.max(
    1,
    Math.ceil(filteredReservas.length / ITEMS_PER_PAGE)
  );
  ...
  const handleDeleteSesion = async (id) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(result.message || "No se pudo eliminar la sesión.");
      }
  ...
  const handleDeleteReserva = async (id) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(result.message || "No se pudo eliminar la reserva.");
      }
        ...
        <div className="px-4 md:px-5 py-3 border-b border-gray-800">
          <div className="relative max-w-md">
            ...
            <input
              ...
              placeholder={
                activeTab === "agenda"
                  ? "Buscar por disciplina, instructor o fecha..."
                  : "Buscar por folio, socio o fecha..."
              }
              value={searchQuery}
                  ...
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-gray-800">
                      {paginatedReservas.map((r, idx) => (
                        ...
                        >
                          <td className="px-4 py-3 text-sm font-mono text-blue-400 font-semibold">
                            {r.folio_reserva}
                          </td>
                          <td className="px-4 py-3 text-sm text-gray-300">
                            {r.socio
                ...
                <div className="md:hidden divide-y divide-gray-800">
                  {paginatedReservas.map((r) => (
                    <div key={r.id_reserva} className="p-4 space-y-2 hover:bg-gray-800/20 transition-colors">
                      <div className="flex items-start justify-between">
                        <div>
                          <h3 className="font-semibold text-sm text-blue-400 font-mono">
                            {r.folio_reserva}
                          </h3>
                          <p className="text-xs text-gray-500">
                            {r.socio
                ...
                {(() => {
                  ...
                  if (isAgenda) {
                    ...
                    return (
                      <>
                        ...
                        <button
                          onClick={() => {
                            handleDeleteSesion(s.id_sesion);
                            setActiveMenu(null);
                            setMenuPos(null);
                          }}
                  ...
                  return (
                    <>
                      <button
                        onClick={() => {
                          handleViewReserva(r.id_reserva);
                          setActiveMenu(null);
                          setMenuPos(null);
                        }}
                        className="flex w-full items-center px-4 py-2 text-sm text-gray-300 hover:bg-white/10 transition"
                      ...
                      <button
                        onClick={() => {
                          handleDeleteReserva(r.id_reserva);
                          setActiveMenu(null);
                          setMenuPos(null);
                        }}
                        className="flex w-full items-center px-4 py-2 text-sm text-red-400 hover:bg-white/10 transition"

...
const Facilities = () => {
  ...
  const parseJsonResponse = async (response) => {
    ...
    try {
      ...
      throw new Error(
        `El servidor no respondió con JSON válido. HTTP ${response.status}`
      );
    }
  };
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      items = items.filter((f) => {
        const nombre = f.nombre_especifico || "";
        const superficie = f.tipo_superficie || "";
        const ubicacion = f.ubicacion || "";

        return (
          nombre.toLowerCase().includes(q) ||
          superficie.toLowerCase().includes(q) ||
          ubicacion.toLowerCase().includes(q)
        );
      });
    }
  ...
  const getFacilities = useCallback(async () => {
    ...
    try {
      ...
      setError(error.message || "Error cargando instalaciones.");
    } finally {
      setLoading(false);
    }
  }, [token, authHeaders]);
  ...
  const handleViewDetails = async (id) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.message || "No se pudieron obtener los detalles."
        );
      }
      ...
      setError(error.message || "Error al obtener detalles.");
    }
  };
          ...
          <>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {f.tipo_superficie || "No definida"} · {f.capacidad_max}{" "}
                        pax
                      </p>
                    </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 md:px-5 py-3 border-t border-gray-800">
                <p className="text-xs text-gray-500">
                  {filteredAndSorted.length} instalaciones — Pág. {currentPage}{" "}
                  de {totalPages}
                </p>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5">
        ...
        <span
          className={`${
            isActive ? "text-yellow-400" : "text-gray-600 group-hover:text-gray-400"
          } transition-colors`}
        >
          {isActive ? (
